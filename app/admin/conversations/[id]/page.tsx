...
async function setStatusAction(formData: FormData) {
  ...
  await prisma.conversation.update({
    ...
    data: {
      status,
      cancelledAt: status === ConversationStatus.cancelled ? new Date() : null,
    },
  });
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
        <div>
          ...
          <div style={{ color: "#666", marginTop: 6 }}>
            ...
            <b>{conversation.status}</b> • failCount: <b>{conversation.failCount}</b>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          <form action={setStatusAction}>
            ...
            <input type="hidden" name="status" value={ConversationStatus.active} />
            <button style={btnStyle} type="submit">Activer</button>
          </form>

          <form action={setStatusAction}>
            <input type="hidden" name="id" value={conversation.id} />
            <input type="hidden" name="status" value={ConversationStatus.completed} />
            <button style={btnStyle} type="submit">Terminer</button>
          </form>

          <form action={setStatusAction}>
            <input type="hidden" name="id" value={conversation.id} />
            <input type="hidden" name="status" value={ConversationStatus.cancelled} />
